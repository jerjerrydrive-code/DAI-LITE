...
private val Context.dataStore: DataStore<Preferences> by preferencesDataStore(name = "dai_settings")
...
class SettingsStore @Inject constructor(
    ...
    // AI provider API base URL (OpenAI/OpenRouter-compatible chat/completions + models endpoints)
    ...
    suspend fun setApiBaseUrl(url: String) {
        context.dataStore.edit { preferences ->
            ...
            if (trimmed.isEmpty()) {
                ...
                preferences[API_BASE_URL] = trimmed
            }
        }
    }
    ...
    val defaultProjectPath: Flow<String> = context.dataStore.data.map { preferences ->
        preferences[DEFAULT_PROJECT_PATH] ?: "/ext/apps_data/dai"
    }
    ...
    companion object {
        // Default AI provider: NVIDIA's free-tier OpenAI-compatible API base,
        // matching the nvapi- key the user already holds — no extra signup needed.
        // Any OpenAI/OpenRouter-compatible gateway works here (e.g. OpenRouter:
        // https://openrouter.ai/api/v1, BazaarLink: https://bazaarlink.ai/api/v1)
        // — just change it in Settings.
        const val DEFAULT_BASE_URL = "https://integrate.api.nvidia.com/v1"
        // Small, fast free-tier NVIDIA model — works out of the box with the default base URL above.
        const val DEFAULT_MODEL = "meta/llama-3.2-3b-instruct"
