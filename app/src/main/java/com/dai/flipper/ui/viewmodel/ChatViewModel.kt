package com.dai.flipper.ui.viewmodel
...
import com.dai.flipper.ble.FlipperBleService
...
import com.dai.flipper.glasses.GlassesIntegration
import com.dai.flipper.glasses.GlassesMessage
import com.dai.flipper.glasses.MessageType
import com.dai.flipper.voice.SpeechRecognitionHelper
...
class ChatViewModel @Inject constructor(
    ...
    val conversationState: StateFlow<ConversationState> = daiAgent.conversationState
    ...
    fun sendMessage() {
        ...
        viewModelScope.launch {
            try {
                ...
                // DaiAgent.sendMessage() has its own try-catch that resets
    ...
    fun approveAction(approvalId: String? = null) {
        ...
        viewModelScope.launch {
            try {
                daiAgent.continueAfterApproval(targetApprovalId, approved = true)
            } finally {
    ...
    fun rejectAction(approvalId: String? = null) {
        ...
        viewModelScope.launch {
            try {
                daiAgent.continueAfterApproval(targetApprovalId, approved = false)
            } finally {
                approvalDecisionInFlight = false
            }
        }
    ...
    fun clearConversation() {
        _pendingImages.value = emptyList()
        daiAgent.clearConversation()
    }

    fun startNewSession(deviceName: String? = null) {
        _pendingImages.value = emptyList()
        daiAgent.startNewSession(deviceName)
    }

    val sessionHistory: StateFlow<List<ChatSessionSummary>> =
        daiAgent.getSessions()
            .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    fun loadSession(sessionId: String) {
        viewModelScope.launch {
            daiAgent.loadSession(sessionId)
        }
    }

    fun deleteSession(sessionId: String) {
        viewModelScope.launch {
            daiAgent.deleteSession(sessionId)
        }
    }
}
