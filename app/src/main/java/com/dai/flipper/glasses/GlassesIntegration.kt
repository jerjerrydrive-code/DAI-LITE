package com.dai.flipper.glasses

import android.util.Log
import com.dai.flipper.ai.OpenRouterClient
import com.dai.flipper.ai.PhotoCaptureCallback
import com.dai.flipper.ai.DaiAgent
 ...
 *
 * Handles:
 * - Voice transcriptions from glasses → DaiAgent as user messages
 * - Camera photos from glasses → DaiAgent as image attachments
 * - AI responses from DaiAgent → glasses for TTS + HUD display
 ...
 * - "Hey Dai" wake word commands → immediate execution
...
class GlassesIntegration @Inject constructor(
    ...
    companion object {
        ...
        private const val SAILOR_SPEECH_SUMMARY_PROMPT =
            "You are Dai, a foul-mouthed hacker voice assistant on smart glasses. " +
            "Compress the user's message into a single spoken sentence. " +
    ...
    fun connect(bridgeUrl: String) {
        ...
        startListeners()
        daiAgent.setPhotoCaptureCallback(photoCaptureCallback)
    }
    ...
    fun disconnect() {
        ...
        daiAgent.setPhotoCaptureCallback(null)
    ...
    private fun startListeners() {
        ...
        responseListenerJob = scope.launch {
            daiAgent.conversationState.collect { state ->
    ...
    /**
     * Voice transcription from glasses mic → send as user message to DaiAgent.
    ...
    /**
     * Explicit voice command from "Hey Dai" wake word.
     ...
     * Validates the approval is still live in DaiAgent before acting.
     *
    ...
    private suspend fun tryHandleApprovalVoice(text: String): Boolean {
        approvalMutex.withLock {
            ...
            val currentApproval = daiAgent.conversationState.value.pendingApproval
            ...
            daiAgent.continueAfterApproval(approvalId, approved = approved)
            return true
        }
    }

    /**
     * Send a message to DaiAgent, attaching any pending glasses photo.
    ...
    private suspend fun sendWithPendingPhoto(text: String) {
        ...
        if (photo != null) {
            ...
            daiAgent.sendMessage(userMessage = text)
            // Skip verbose status — "Thinking..." will narrate via progress listener
        }
    }
    ...
    /**
     * Watch DaiAgent conversation state for new assistant messages
    ...
    /**
     * Callback registered on DaiAgent so the LLM can request photos via
     * the request_photo tool action.
     *
     * Flow:
     *   1. LLM calls request_photo → DaiAgent calls this callback
